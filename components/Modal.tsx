    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative w-full max-w-[1200px] bg-white p-6 rounded shadow-lg z-10">
                {title && <h2 className="text-xl absolute top-0 left-0 px-5 py-5">{title}</h2>}
                <div className="h-3/4 max-h-[500px] overflow overflow-y-scroll mt-10 mb-20">
                    {children}
                </div>
                <div
                    className="flex items-center w-full border-t-1 border-stone-200 pt-5 absolute bottom-0 right-0 px-5 py-5 bg-white">
                    <div className="ml-auto space-x-7">
                        <button className="px-4 py-2 cursor-pointer" onClick={() => onClose(false)}>{closeBtnText}</button>
                        {onSubmit && <button className="px-4 py-2 bg-blue-700 text-white rounded cursor-pointer" onClick={handleSubmit}>{submitBtnText}</button>}
                    </div>
                </div>
            </div>
        </div>
